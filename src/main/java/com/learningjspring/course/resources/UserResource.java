...
public class UserResource {
    ...
    public ResponseEntity<User> insertUser(@RequestBody User user) {
        ...
        return ResponseEntity.created(uri).body(user); //Retornando a resposta HTTP 201 CREATED
    }
    
    @DeleteMapping(value = "/{id}") //notação padrão REST deleção HTTP
    public ResponseEntity<Void> deleteUser(@PathVariable Long id) {
        userService.deleteUser(id);
        return ResponseEntity.noContent().build(); //resposta que não tem conteúdo(Já retorna a resposta HTTP correta)
    }
